...
void Component::invokeEx( Project& project, FCO& currentFCO, const std::set<FCO>& setSelectedFCOs, long lParam )
{
	...
	if (rf)
	{
		...
		{
			ProofTreeAST::Parser* parser = new ProofTreeAST::Parser();
			try
			{	
				ProofTreeAST::Sentence* root = (ProofTreeAST::Sentence*)parser->parse(&(ProofTreeBON::Tree)*it);
				std::stringstream ss;
				ss << "Processing formula: " << root->toString().c_str();
				Console::Out::WriteLine(_T(ss.str().c_str()));
				Tableaux::Solver* solver = new Tableaux::Solver();
				Tableaux::Tableaux* proofTree = solver->solve(root);
				
				// HTML
				ss.str(std::string());
				ss.clear();
				ss << project->getName() << ".html";
				Tableaux::TableauxRenderer* renderer = new Tableaux::TableauxRenderer();
				std::string fileName = ss.str();;
				{
					...
					if (outfile.is_open())
					{
						outfile << renderer->renderHTML(proofTree, solver->getOpenBranches());
					}
					outfile.flush();
					outfile.close();
				}
				...
				fileName = ss.str();
				{
					...
					if (outfile.is_open())
					{
						outfile << renderer->renderXML(proofTree);
					}
					outfile.flush();
					outfile.close();
				}
			...
			{
				std::stringstream ss;
				ss << "Parsing error: " << e.getErrorMessage() << ". Did you forget to check all constraints?";
				GMEConsole::Console::Out::WriteLine(ss.str().c_str());
			}
		}
	}
